...
# update haulinfo and add startmonth column
haul_info <- copy(haul_info)[haul_id %in% raw$haul_id]
haul_info %<>% merge(unique(raw[,.(survey, startmonth)]), all.x=TRUE, by="survey")


# STANDARDIZE SURVEY FOOTPRINTS (M. Pinsky)

FishGlob <- copy(raw)[,longitude_s := ifelse(longitude > 150,(longitude-360),(longitude))]
...
survs <- FishGlob.dg[, unique(survey)]
year_cell_count.dt <- NULL
for(i in 1:length(survs)) {
  tmp <- FishGlob.dg[survey==survs[i]]
...
temp <- FishGlob.dg[, .(nhaul = length(unique(haul_id))), by = .(cell, year, survey)]
