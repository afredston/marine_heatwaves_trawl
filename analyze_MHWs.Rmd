...
library(lme4)
lme_wtMtDiffProp_anomIntC <- inner_join(namerSummary, mhwSurvSummary) %>% 
...
lme_wtMtAnomProp_anomIntC <- inner_join(namerSummary, mhwSurvSummary) %>% 
...
lme_wtMtDiffProp_mhwYesNo <- inner_join(namerSummary, mhwSurvSummary) %>% 
  mutate(mhwYesNo = ifelse(anomDays > 0, "yes", "no")) %>% 
...
lme_wtMtAnomProp_mhwYesNo <- inner_join(namerSummary, mhwSurvSummary) %>% 
  mutate(mhwYesNo = ifelse(anomDays > 0, "yes", "no")) %>% 
...
lm_spp_wtMtDiffProp_anomIntC_ref_year <- inner_join(namerSppSummary, mhwSurvSummary) %>% 
  filter(!is.na(wtMtDiffProp),
         wtMtDiffProp < Inf) %>% # because the proportion becomes Inf if previous year was 0
...
lm_spp_wtMtDiffProp_ref_year <- inner_join(namerSppSummary, mhwSurvSummary) %>% 
  filter(!is.na(wtMtDiffProp),
         wtMtDiffProp < Inf) %>% # because the proportion becomes Inf if previous year was 0
