...
```{r MHWdata, eval=TRUE, echo=FALSE, results=FALSE, message=FALSE, warning=FALSE}
...
```{r trawldata, eval=TRUE, echo=FALSE, results=FALSE, message=FALSE, warning=FALSE}
...
ggMhwNamerCpue2_save <- namerSummary %>% 
  filter(year>=1982) %>% 
  left_join (mhwSummary %>% filter(region %in% namerRegs), by=c("region","year")) %>% 
  ggplot(aes(x=anomIntC, y=wtMtDiffProp)) + 
  geom_point() +
  ...
  NULL
ggsave(ggMhwNamerCpue2_save, filename=here("results","scatter_plots_MHW_CPUE_north_america.png"), dpi=160, width=8, height=4)
...
```{r namer-mhw-5-spp, fig.cap="MHW Cumulative Mean Intensity vs. Single-Species Change in Total CPUE", fig.height=8, fig.scale=1.2, eval=TRUE, echo=FALSE, results=TRUE, message=FALSE, warning=FALSE}
...
```{r namer-mhw-yearly-boxplot1, fig.cap="CPUE Proportional Anomaly in MHW vs. non-MHW years", fig.height=8, eval=TRUE, echo=FALSE, results=TRUE, message=FALSE, warning=FALSE}
...
```{r namer-mhw-yearly-boxplot2, fig.cap="CPUE Proportional Year-Over-Year Difference in MHW vs. non-MHW years", fig.height=8, eval=TRUE, echo=FALSE, results=TRUE, message=FALSE, warning=FALSE}
...
Fitting a mixed-effects model to all the species-level data (not just top species):

```{r lme, eval=TRUE, echo=FALSE, results=TRUE, message=FALSE, warning=FALSE, results='asis'}
library(lme4)
library(broom.mixed)
library(kableExtra)
...
lme1 <- lmer(wtMtDiffPropAbs ~ sstYrAnom + (1|spp), data = namerSppMhw) %>% tidy()
