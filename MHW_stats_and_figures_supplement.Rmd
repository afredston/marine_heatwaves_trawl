...
# Appendix: Robustness to metrics of biomass and MHWs

```{r fig-abs-biomass, fig.cap="Caption"}
...
```{r fig-intensity, fig.cap="Caption"}
...
# Appendix: Robustness to metrics of community change and variability

# Appendix: Predictors of regional biomass change
...
# Appendix: Predictors of species-level change

Our focus on net change in biomass in temperate marine fish assemblages
may obscure important species-level changes in abundance. One way we
address this is our analysis (presented in the main text) of shifts in
community composition and Community Temperature Index. Here, we also
explore patterns in species-level biomass change in the context of
marine heatwaves.

# Appendix: Case study of the Northeast Pacific

The Northeast Pacific 2014-2016 is perhaps the best-documented and most intense MHW in modern history. 
...
boop <- survey_spp_summary %>% 
  ...
  bind_rows(survey_spp_summary %>% filter(survey %in% c('EBS','GOA','AI'), !spp %in% topspp$spp) %>% group_by(ref_yr, year, survey) %>% summarise(wt_mt = sum(wt_mt)) %>% mutate(spp="Others")) %>% 
  inner_join(mhw_summary_sat_sst_5_day, by="ref_yr") %>% 
