...
kbl(depth.tbl, booktabs = TRUE, caption = "Null (intercept-only) model and model of depth response to MHWs. Linear model was fitted to the weighted mean depth of the fish assemblage (scaled and centered within surveys) using MHW severity in °C days (scaled and centered within surveys).",
    escape = FALSE) %>%
  kable_styling(font_size = 8, latex_options = c("striped"))
...
lm.cti <- lm(cti_log_scale ~ anom_sev_scale, data = modeldat)
lm.fixed.cti <- lm(cti_log_scale ~ anom_sev_scale + factor(survey), data = modeldat)
gam.cti <- gam(cti_log_scale ~ s(anom_sev_scale), data = modeldat )
gam.re.cti <- gam(cti_log_scale ~ s(anom_sev_scale) + s(survey, bs="re"), data = modeldat  %>% mutate(survey = as.factor(survey)))
...
```{r fig-mhw-point-by-cpue-method, fig.cap="Alternate version of Fig. 2 from the main text using different metrics of biomass change: mean biomass (used in the main text), median biomass, mean abundance, and median abundance. Points represent log ratios of each metric in a survey from one year to the next. The fitted lines are linear regressions. The shaded areas are 95% confidence intervals. Survey names are listed in Supp. Tab. 1. The Northeast US survey is omitted because it does not have abundance data recorded.", fig.width=9, fig.height=7}
...
```{r fig-abs-biomass, fig.cap="MHW severity and absolute value of biomass log ratio. The fitted line is a linear regression. The shaded area is its 95% confidence interval."}
...
```{r fig-depth-point, fig.cap="Fish assemblage depth change (log ratio) and MHW severity. The fitted line is a linear regression. The shaded area is its 95% confidence interval."}
...
```{r fig-lat-point, fig.cap="Biomass change (log ratio) and MHW severity, color-coded by median latitude of each survey region. The fitted line is a linear regression. The shaded area is its 95% confidence interval."}
med_lat <- haul_info %>% group_by(survey) %>% summarise(med_lat = median(latitude))
...
```{r fig-lag-point, fig.cap="Biomass change (log ratio) and MHW severity calculated the preceding year as in the main text (0-12 months), a one-year lag (12-24 months), and a two-year lag (24-36 months). Fitted lines are linear regressions. Shaded areas are 95% confidence intervals."}
...
```{r fig-spp-tl, fig.cap="Biomass log ratio and MHW severity grouped by trophic level of each taxon. Trophic levels are binned (2-3, 3-4, and 4-5). Fitted lines are linear regressions. Shaded areas are 95% confidence intervals."}
...
```{r fig-spp-feed, fig.cap="Biomass log ratio and MHW severity grouped by feeding mode of each taxon. Fitted lines are linear regressions. Shaded areas are 95% confidence intervals."}
...
```{r fig-spp-hab, fig.cap="Biomass log ratio and MHW severity grouped by habitat preference of each taxon. Fitted lines are linear regressions. Shaded areas are 95% confidence intervals."}
