...
# ±
kbl(wt.tbl, booktabs = TRUE, caption = "Models of biomass response to MHWs. All models were fitted to biomass log ratio values (scaled and centered within surveys) using MHW duration in days (scaled and centered within surveys). Model names correspond to: null (intercept-only) model, linear model, linear model including survey as a fixed effect, generalized additive model (GAM), and GAM including survey as a random effect.",
    escape = FALSE) %>%
  kable_styling(font_size = 8, latex_options = c("striped"))
```
...
coef.gompertz <- c('NA',paste0(format(round(gompertz.summary["anom_days_scale","Estimate"] , digits=3), nsmall=2)," ± ", format(round(gompertz.summary["anom_days_scale","Std. Error"] , digits=3), nsmall=2)) )

p.gompertz <- c('NA',paste0(format(round(gompertz.summary["anom_days_scale","Pr(>|z|)"] , digits=3), nsmall=2)))
...
```{r tbl-models-cti}
null.cti <- lm(cti_log_scale ~ 1, data = modeldat)
