...
converted into an index of catch per unit effort (here, kg / km\^2). To
estimate the fish biomass from each sample, the minimum required
information is: taxa specific weight or abundance at length data, swept
area or information to estimate it from sampling duration, sampled
distance and gear opening. When species weight needed to be estimated
from abundance at length data for ICES surveys, length-weight
...
survey-years. A total of 18 surveys were incorporated, with a total of
202,819 hauls covering 2088 taxa before spatio-temporal standardization
of each survey region. 80% of those taxa were identified to species, 12%
to genus, and the remaining 8% to higher taxonomic levels. We used all
of the taxa for biomass analyses but only the species-level records for
community analyses.
...
(@fredston2021). We explored whether species taxonomy, species traits,
and regional identity help to predict species-level biomass change at
all and in the context of marine heatwaves. All fish species traits were
...
```{r fig-nep-spp, fig.cap="Biomass trends and historical MHWs in Northeast Pacific surveys. The top five taxa by biomass are highlighted in each survey. Shaded grey rectangles represent MHWs that occurred in the preceding survey-year (e.g., 2015 in the Eastern Bering Sea time-series corresponds to the survey that began in June 2015, and MHW data from June 2014 - May 2015). Eastern Bering Sea and West Coast US are annual surveys. Gulf of Alaska is a biennial survey."}
topspp_nep <- survey_spp_summary %>% 
  filter(survey %in% c('EBS','GOA','WCANN','DFO-QCS')) %>% 
  group_by(survey, spp) %>% 
...
survey_spp_summary %>% 
  mutate(id = paste0(survey, spp)) %>% 
  filter(id %in% topspp_nep$id) %>% 
  bind_rows(survey_spp_summary %>% filter(survey %in% topspp_nep$survey, !spp %in% topspp_nep$spp) %>% group_by(ref_yr, year, survey) %>% summarise(wt_mt = sum(wt_mt)) %>% mutate(spp="Others")) %>% # get "others" group 
  inner_join(mhw_summary_sat_sst_5_day, by="ref_yr") %>% 
...
```{r fig-nwa-spp, fig.cap="Biomass trends and historical MHWs in Northwest Atlantic surveys. The top five taxa by biomass are highlighted in each survey. Shaded grey rectangles represent MHWs that occurred in the preceding survey-year (e.g., 2015 in the Eastern Bering Sea time-series corresponds to the survey that began in June 2015, and MHW data from June 2014 - May 2015). Eastern Bering Sea and West Coast US are annual surveys. Gulf of Alaska is a biennial survey."}
topspp_nwa <- survey_spp_summary %>% 
  filter(survey %in% c('NEUS','GSL-S','GMEX','SEUS','SCS')) %>% 
  group_by(survey, spp) %>% 
  summarise(tot = sum(wt_mt)) %>% 
  arrange(-tot) %>% 
  slice(1:5) %>% 
...
survey_spp_summary %>% 
  mutate(id = paste0(survey, spp)) %>% 
  filter(id %in% topspp_nwa$id) %>% 
  bind_rows(survey_spp_summary %>% filter(survey %in% topspp_nwa$survey, !spp %in% topspp_nwa$spp) %>% group_by(ref_yr, year, survey) %>% summarise(wt_mt = sum(wt_mt)) %>% mutate(spp="Others")) %>% # get "others" group 
  inner_join(mhw_summary_sat_sst_5_day, by="ref_yr") %>% 
  left_join(survey_names) %>% 
  ggplot() +
  geom_area(aes(x=year, y=wt_mt, color=spp, fill=spp, group=spp)) + 
...
```{r fig-nea-spp, fig.cap="Biomass trends and historical MHWs in Northeast Atlantic surveys. The top five taxa by biomass are highlighted in each survey. Shaded grey rectangles represent MHWs that occurred in the preceding survey-year (e.g., 2015 in the Eastern Bering Sea time-series corresponds to the survey that began in June 2015, and MHW data from June 2014 - May 2015). Eastern Bering Sea and West Coast US are annual surveys. Gulf of Alaska is a biennial survey."}
topspp_nea <- survey_spp_summary %>% 
  filter(survey %in% c('Nor-BTS','NS-IBTS','SWC-IBTS','EVHOE','FR-CGFS','BITS','IE-IGFS','PT-IBTS','NIGFS')) %>% 
  group_by(survey, spp) %>% 
  summarise(tot = sum(wt_mt)) %>% 
  arrange(-tot) %>% 
  slice(1:5) %>% 
...
survey_spp_summary %>% 
  mutate(id = paste0(survey, spp)) %>% 
  filter(id %in% topspp_nea$id) %>% 
  bind_rows(survey_spp_summary %>% filter(survey %in% topspp_nea$survey, !spp %in% topspp_nea$spp) %>% group_by(ref_yr, year, survey) %>% summarise(wt_mt = sum(wt_mt)) %>% mutate(spp="Others")) %>% # get "others" group 
  inner_join(mhw_summary_sat_sst_5_day, by="ref_yr") %>% 
  left_join(survey_names) %>% 
  ggplot() +
  geom_area(aes(x=year, y=wt_mt, color=spp, fill=spp, group=spp)) + 
