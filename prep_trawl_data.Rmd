...
# Europe
datras_raw <- read_csv(here("raw-data","DATRAS_clean_Aug2021.csv"))%>% 
  select(where(~!all(is.na(.x)))) #drop FISHGLOB columns that are all NAs for this survey
...
datras_cpue <- rbind(swc_ibts_raw_zeros, rockall_raw_zeros, pt_ibts_raw_zeros, ns_ibts_raw_zeros, nigfs_raw_zeros, ie_igfs_raw_zeros, fr_cgfs_raw_zeros, evhoe_raw_zeros, bits_raw_zeros) %>% 
  mutate(spp=accepted_name, region=survey) %>% 
