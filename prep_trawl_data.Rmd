...
nor_intervals <- nor_cpue %>% 
  select(region, year) %>% 
  distinct() %>% 
  group_by(region) %>% 
  arrange(year) %>% 
  mutate(year_interval = time_length(interval(year, lag(year))))

datras_intervals <- datras_cpue %>% 
  select(region, year) %>% 
  distinct() %>% 
  group_by(region) %>% 
  arrange(year) %>% 
  mutate(year_interval = interval(lubridate::ym(paste0(year,"-01"),lubridate::ym( paste0(lag(year),"-01")))))

namer_intervals <- namer_cpue %>% 
  select(region, year) %>% 
  distinct() %>% 
  group_by(region) %>% 
  arrange(year) %>% 
