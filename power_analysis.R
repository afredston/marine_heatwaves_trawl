...
mhw_summary_oisst_d_5_day <- read.csv(here("processed-data","MHW_oisst_5_day_threshold.csv")) # MHW summary stats from satellite SST record, defining MHWs as events >=5 days
mhw_summary_glorys_d_5_day <- read.csv(here("processed-data","MHW_glorys_5_day_threshold.csv")) # MHW summary stats from satellite SST record, defining MHWs as events >=5 days

survey_summary_oisst <-read.csv(here("processed-data","survey_biomass_with_CTI.csv")) %>% inner_join(mhw_summary_oisst_d_5_day)
survey_summary_glorys <-read.csv(here("processed-data","survey_biomass_with_CTI.csv")) %>% inner_join(mhw_summary_glorys_d_5_day)
dat_ls <- list(survey_summary_oisst,survey_summary_glorys)
...
for(z in 1:2){
  datasource <- ifelse(z==1, 'oisst','glorys')
  ...
  iters <- 1000
  trialyrs <- c(seq(10, 40, 1), seq(50, 200, 10)) # glorys mean years/survey is ~20; oisst is ~24
  ...
  saveRDS(pwrout_yrs, file=here("processed-data",paste0("pwrout_yrs_",datasource,".rds")))
  ...
  saveRDS(sim_test_yrs, file=here("processed-data",paste0("sim_test_yrs_",datasource,".rds")))
  saveRDS(sim_test_summ_yrs, file=here("processed-data",paste0("sim_test_summ_yrs_",datasource,".rds")))
  ...
  saveRDS(pwrout_gamma, file=here("processed-data",paste0("pwrout_gamma_",datasource,".rds")))
  ...
  saveRDS(sim_test_gamma, file=here("processed-data",paste0("sim_test_gamma_",datasource,".rds")))
  ...
  saveRDS(sim_test_summ_gamma, file=here("processed-data",paste0("sim_test_summ_gamma_",datasource,".rds")))
  
} # close for loop over OISST/GLORYS
