...
ggsave(ggsummary, filename=here("results","line_plots_all_CPUE_MHW.png"), dpi=600, width=8, height=5)
```
...
left_join(region_spp_summary, mhwSurvSummary) %>% 
...
```{r lme_wtMtDiffProp_anomIntC}
library(lme4)
lme_wtMtDiffProp_anomIntC <- inner_join(region_summary, mhwSurvSummary) %>% 
  lmer(wtMtDiffProp ~ anomIntC + 1|region, data=.)

summary(lme_wtMtDiffProp_anomIntC)
```


```{r lme_wtMtAnomProp_anomIntC}
lme_wtMtAnomProp_anomIntC <- inner_join(region_summary, mhwSurvSummary) %>% 
  lmer(wtMtAnomProp ~ anomIntC + 1|region, data=.)

summary(lme_wtMtAnomProp_anomIntC)
```

```{r lme_wtMtDiffProp_mhwYesNo}
lme_wtMtDiffProp_mhwYesNo <- inner_join(region_summary, mhwSurvSummary) %>% 
  lmer(wtMtDiffProp ~ mhwYesNo + 1|region, data=.)

summary(lme_wtMtDiffProp_mhwYesNo)
```

```{r lme_wtMtAnomProp_mhwYesNo}
lme_wtMtAnomProp_mhwYesNo <- inner_join(region_summary, mhwSurvSummary) %>% 
  lmer(wtMtAnomProp ~ mhwYesNo + 1|region, data=.)

summary(lme_wtMtAnomProp_mhwYesNo)
```
...
lm_wtMtDiffProp_mhwYesNo <- inner_join(region_summary, mhwSurvSummary) %>% 
...
lm_wtMtAnomProp_mhwYesNo <- inner_join(region_summary, mhwSurvSummary) %>% 
...
lm_wtMtAnomProp_mhwYesNo_region <- inner_join(region_summary, mhwSurvSummary) %>% 
...
lm_wtMtDiffProp_mhwYesNo_region <- inner_join(region_summary, mhwSurvSummary) %>% 
...
lm_spp_wtMtDiffProp_mhwYesNo_region <- inner_join(region_spp_summary, mhwSurvSummary) %>% 
  filter(!is.na(wtMtDiffProp),
         wtMtDiffProp < Inf) %>% # because the proportion becomes Inf if previous year was 0
  lm(wtMtDiffProp ~ mhwYesNo + mhwYesNo * region, data=.)
...
lm_spp_wtMtAnomProp_mhwYesNo_region <- inner_join(region_spp_summary, mhwSurvSummary) %>% 
  filter(!is.na(wtMtAnomProp),
         wtMtAnomProp < Inf) %>% # because the proportion becomes Inf if previous year was 0
  lm(wtMtAnomProp ~ mhwYesNo + mhwYesNo * region, data=.)
...
lm_wtMtDiffProp_anomIntC <- inner_join(region_summary, mhwSurvSummary) %>% 
...
lm_wtMtAnomProp_anomIntC <- inner_join(region_summary, mhwSurvSummary) %>% 
...
lm_wtMtDiffProp_anomIntC_region <- inner_join(region_summary, mhwSurvSummary) %>% 
...
lm_wtMtAnomProp_anomIntC_region <- inner_join(region_summary, mhwSurvSummary) %>% 
...
lm_wtMtAnomProp_anomDays_region <- inner_join(region_summary, mhwSurvSummary) %>% 
...
lm_wtMtDiffProp_anomDays_region <- inner_join(region_summary, mhwSurvSummary) %>% 
...
lm_wtMtAnomProp_anomSev_region <- inner_join(region_summary, mhwSurvSummary) %>% 
...
lm_wtMtDiffProp_anomSev_region <- inner_join(region_summary, mhwSurvSummary) %>% 
...
```{r lm_spp_wtMtDiffProp_anomIntC_ref_year}
lm_spp_wtMtDiffProp_anomIntC_ref_year <- inner_join(region_spp_summary, mhwSurvSummary) %>% 
  filter(!is.na(wtMtDiffProp),
         wtMtDiffProp < Inf) %>% # because the proportion becomes Inf if previous year was 0
  lm(wtMtDiffProp ~ anomIntC + anomIntC * ref_year, data=.)

summary(lm_spp_wtMtDiffProp_anomIntC_ref_year)

tidy(lm_spp_wtMtDiffProp_anomIntC_ref_year) %>% 
  filter(p.value < 0.10)
```


```{r lm_spp_wtMtDiffProp_ref_year}
lm_spp_wtMtDiffProp_ref_year <- inner_join(region_spp_summary, mhwSurvSummary) %>% 
  filter(!is.na(wtMtDiffProp),
         wtMtDiffProp < Inf) %>% # because the proportion becomes Inf if previous year was 0
  lm(wtMtDiffProp ~ ref_year, data=.)

summary(lm_spp_wtMtDiffProp_ref_year)

```

```{r lm_spp_wtMtDiffProp_anomIntC_region}
lm_spp_wtMtDiffProp_anomIntC_region <- inner_join(region_spp_summary, mhwSurvSummary) %>% 
  filter(!is.na(wtMtDiffProp),
         wtMtDiffProp < Inf) %>% # because the proportion becomes Inf if previous year was 0
  lm(wtMtDiffProp ~ anomIntC + anomIntC * region, data=.)
...
lm_spp_wtMtDiffProp_anomDays_region <- inner_join(region_spp_summary, mhwSurvSummary) %>% 
  filter(!is.na(wtMtDiffProp),
         wtMtDiffProp < Inf) %>% # because the proportion becomes Inf if previous year was 0
  lm(wtMtDiffProp ~ anomDays + anomDays * region, data=.)
...
lm_spp_wtMtDiffProp_anomSev_region <- inner_join(region_spp_summary, mhwSurvSummary) %>% 
  filter(!is.na(wtMtDiffProp),
         wtMtDiffProp < Inf) %>% # because the proportion becomes Inf if previous year was 0
  lm(wtMtDiffProp ~ anomSev + anomSev * region, data=.)
...
lm_spp_wtMtAnomProp_anomIntC_region <- inner_join(region_spp_summary, mhwSurvSummary) %>% 
  filter(!is.na(wtMtAnomProp),
         wtMtAnomProp < Inf) %>% # because the proportion becomes Inf if previous year was 0
  lm(wtMtAnomProp ~ anomIntC + anomIntC * region, data=.)
...
lm_spp_wtMtAnomProp_anomDays_region <- inner_join(region_spp_summary, mhwSurvSummary) %>% 
  filter(!is.na(wtMtAnomProp),
         wtMtAnomProp < Inf) %>% # because the proportion becomes Inf if previous year was 0
  lm(wtMtAnomProp ~ anomDays + anomDays * region, data=.)
...
lm_spp_wtMtAnomProp_anomSev_region <- inner_join(region_spp_summary, mhwSurvSummary) %>% 
  filter(!is.na(wtMtAnomProp),
         wtMtAnomProp < Inf) %>% # because the proportion becomes Inf if previous year was 0
  lm(wtMtAnomProp ~ anomSev + anomSev * region, data=.)
