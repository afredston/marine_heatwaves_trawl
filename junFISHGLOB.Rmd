...
```{r namer-mhw-years-cpue-scatter-diff, fig.cap="Biomass Change vs MHW-Years",results='hide', eval=FALSE}
...
```{r namer-spp-anom-coef-boxplot, fig.cap="Distribution of Coefficients for Single-Species Linear Models of Biomass Change on MHW Intensity"}
lm_spp_anom <- left_join(namerSppSummary, mhwSurvSummary) %>% 
  filter(!is.na(wtMtDiffProp),
         wtMtDiffProp < Inf) %>% 
  group_by(spp, region) %>% 
  nest() %>% 
  mutate(
    model = purrr::map(data, ~lm(wtMtAnomProp ~ anomIntC, data = .x)), 
...
lm_spp_anom %>% 
  ...
  facet_wrap(~region) +
  theme_bw() + 
  labs(x="Coefficient of Change in Species Biomass ~ MHW Intensity", y="Density") +
  NULL
```
...
```{r namer-spp-anom-mhwYesNo-log-boxplot, fig.cap="Log Species Proportional CPUE Year-Over-Year Change in MHW and Non-MHW Years", eval=FALSE}
left_join(namerSppSummary, mhwSurvSummary) %>% 
  filter(!is.na(wtMtAnomProp),
